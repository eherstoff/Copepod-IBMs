#######################################################################
# Last updated: 8 November 2018, EMH and MU
...
mean(res.time, na.rm=T)  #% time
sd(res.time, na.rm=T)

mean(apply(res.j,2,sum, na.rm=T)) #n steps
sd(apply(res.j,2,sum, na.rm=T))



#How long until they first find a patch ########
mean(first.visit) #number of steps in model
sd(first.visit)

#Mean % of copes that found HQ patch through time   ########
mean(means)
sd(means)/sqrt(length(means))

#Count of helical swimming in LQ and HQ food  ########
mean(helix.LQ.dat)  #number helices in LQ
sd(helix.LQ.dat)

mean(helix.HQ.dat)  #number helices in HQ
sd(helix.HQ.dat)


#Time spent swimming in HQ and LQ food  ########
mean(helix.LQ_na.dat) #time spent helixing in LQ
sd(helix.LQ_na.dat)

mean(helix.HQ_na.dat) #time spent helixing in HQ
sd(helix.HQ_na.dat)
