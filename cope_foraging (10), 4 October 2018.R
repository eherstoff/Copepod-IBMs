#######################################################################
# Last updated: 4 October 2018, EMH and MU
...
#Get the data together for the last 1000 rows and combine for heatmap (below) #####
raw.dat2 <- res.i[c((timepoints-1000):(timepoints-1)),]

last.nres <- c()
for(i in 1:ncol(raw.dat2)){
  last.nres<-c(last.nres,raw.dat2[,i])
}
...
freq<-cbind(getdim(as.numeric(freq[,1]),grid2),freq) #putting X, Y, nres in each XY cell, Freq in each XY cell, and % time (=tiny fraction!)
...
# Plot a heatmap of final copepod locations  #######
#make a matrix of copepod locations from the last 1000 timepoints
freq.matrix <- matrix(nrow=grid.size, ncol=grid.size, data= last.nres)

plot(freq.matrix[,2]~freq.matrix[,1], xlim=c(0,grid.size), ylim=c(0,grid.size))

require(grDevices) # for colours Can take heat.colors, topo.colors, terrain.colors, rainbow, hsv, & par.
par(mfrow=c(1,2))
#First map where the food is.
image(z=as.matrix(grid2), useRaster=T, col=gray((0:50)/50), main="Heatmap of food location in the model")
box()
#Next, map where copepods are located.
image(z=as.matrix(last.nres), useRaster=T, col=gray((0:50)/50), main="Heatmap of copepod location in the model")
box()
par(mfrow=c(1,1))
...
plotdat <- barplot(test.concat, ylim=c(0,(max(test.concat)+max(test.error))), col=c("darkgoldenrod1", "steelblue"), cex.axis=1.35, cex.lab=1.5,
                   xlab=c("Hi qual patches       Low qual patches"), ylab="Average number of copes in patch type")
